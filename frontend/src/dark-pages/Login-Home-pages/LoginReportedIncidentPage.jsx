...
export const LoginReportedIncidentPage = () => {
  const [incidents, setIncidents] = useState([]); // State to store incident data
  const [entriesPerPage, setEntriesPerPage] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');

  // Fetch data from API or state
  useEffect(() => {
              ...
              <tbody>
                {displayedIncidents.map((incident) => (
                  <tr key={incident.id} className="border-t border-gray-600 hover:bg-gray-700">
                    ...
                    <td className="p-4 border border-gray-600">{incident.incidentDescription}</td>
